...
class GenerateTheme {
    ...
    apply(compiler) {
        ...
        // https://github.com/webpack/webpack.js.org/issues/4106
        compiler.hooks.watchRun.tapAsync(
            "GenerateTheme",
            ({ modifiedFiles }, callback) => {
                const updateFiles = Array.from(modifiedFiles ?? []);
                // 取出更改的 less 文件
                ...
                // 文件中是否使用主题色变量
                const hasThemeVariablesFiles = _.filter(
                    lessFiles,
                    (lessFile) => {
                        const content = fs.readFileSync(lessFile, "utf-8");
                        const usedThemeVariables = _.filter(
                            themeVariables,
                            (themeVariable) => {
                ...
                generateTheme(_.omit(this.options, "outputFilePath")).then(
                    (css) => {
                        if (
                            !isExistCSSFile ||
                            css !== fs.readFileSync(outputFilePath, "utf-8")
                        ) {
